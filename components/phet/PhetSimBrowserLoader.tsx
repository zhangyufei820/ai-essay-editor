      ...
      <section className="rounded-[var(--radius-soft)] border border-emerald-900/10 bg-[var(--paper-50)] p-4 shadow-sm dark:border-[var(--ink-200)]/10 dark:bg-slate-950">
        <div className="grid gap-3 lg:grid-cols-[1fr_160px_160px_160px_160px]">
          {Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="h-10 animate-pulse rounded-[var(--radius-soft)] bg-[var(--ink-50)] dark:bg-emerald-950/40" />
          ))}
        </div>
      </section>
      <section>
        <div className="mb-3 h-6 w-28 animate-pulse rounded bg-[var(--ink-100)] dark:bg-emerald-950/50" />
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="min-h-[360px] animate-pulse rounded-[var(--radius-soft)] border border-emerald-900/10 bg-[var(--paper-50)] dark:border-[var(--ink-200)]/10 dark:bg-slate-950">
              <div className="aspect-[4/3] rounded-t-lg bg-[var(--ink-50)] dark:bg-emerald-950/40" />
