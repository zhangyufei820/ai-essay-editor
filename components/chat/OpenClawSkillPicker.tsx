...
import {
  OPENCLAW_SKILL_CATEGORIES,
  OPENCLAW_SKILLS,
  type OpenClawSkill,
  type OpenClawSkillCategory,
} from "@/lib/openclaw-skills"

type OpenClawSkillPickerProps = {
  open: boolean
  selectedSkillId?: string | null
  onSelect: (skill: OpenClawSkill) => void
  onClose: () => void
}
...
export function OpenClawSkillPicker({
  ...
  const categoryCounts = useMemo(() => {
    return OPENCLAW_SKILLS.reduce<Record<OpenClawSkillCategory, number>>((acc, skill) => {
      acc[skill.category] = (acc[skill.category] ?? 0) + 1
      return acc
    }, {} as Record<OpenClawSkillCategory, number>)
  }, [])
  ...
  const groupedSkills = useMemo(() => {
    return OPENCLAW_SKILL_CATEGORIES.map((category) => ({
      category,
      skills: filteredSkills.filter((skill) => skill.category === category),
    })).filter((group) => group.skills.length > 0)
  }, [filteredSkills])

  const handleSelect = (skill: OpenClawSkill) => {
    setIsMelting(true)
    onSelect(skill)
                ...
                <div className="sticky top-0 z-10 -mx-4 mb-4 border-b border-[var(--paper-100)] bg-[var(--paper-50)]/95 px-4 pb-4 backdrop-blur sm:-mx-6 sm:px-6">
                  <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 lg:max-w-[560px]">
                      {(["全部", ...OPENCLAW_SKILL_CATEGORIES] as const).map((category) => {
                        const isActive = activeCategory === category
                        const label =
                          category === "全部"
                            ? `全部 · ${OPENCLAW_SKILLS.length}`
                ...
                {filteredSkills.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {groupedSkills.map((group) => (
                      <section key={group.category} aria-labelledby={`openclaw-skill-${group.category}`}>
                        <div className="mb-3 flex items-center justify-between gap-3">
                          <h3
                            id={`openclaw-skill-${group.category}`}
