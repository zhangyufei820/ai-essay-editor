...
const ComposingAnimation = () => (
  ...
  >
    ...
    <h3 className="text-lg font-semibold text-[var(--ink-800)] mb-2">AI 正在创作歌词...</h3>
    <p className="text-sm text-[var(--ink-500)]">根据您的创意，生成独特的歌词和风格</p>
...
const LyricsPreviewCard = ({ 
  ...
  >
    ...
    <div className="px-5 py-4 border-b border-[var(--paper-100)] bg-gradient-to-r from-emerald-50 to-teal-50">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-[var(--ink-500)]">请确认歌词内容，满意后点击生成</p>
        </div>
      </div>
    </div>
    ...
    <div className="p-5 space-y-4">
      ...
      {preview.styleTags && preview.styleTags.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {preview.styleTags.map((tag, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[var(--ink-100)] text-[var(--ink-700)] text-xs font-medium"
            >
      ...
      {/* 风格提示词 */}
      {preview.stylePrompt && (
        <div className="p-3 rounded-[var(--radius-sharp)] bg-[var(--paper-100)] border border-[var(--paper-200)]">
          <p className="text-xs font-medium text-[var(--ink-500)] mb-1">风格提示词</p>
          <p className="text-sm text-[var(--ink-700)] leading-relaxed">{preview.stylePrompt}</p>
        </div>
      )}
      
      {/* 歌词 */}
      <div className="p-4 rounded-[var(--radius-sharp)] bg-[var(--paper-50)] border border-[var(--paper-200)] max-h-[300px] overflow-y-auto">
        <div className="flex items-center gap-2 mb-3">
          <Mic2 className="h-4 w-4 text-[var(--ink-500)]" />
          <span className="text-sm font-medium text-[var(--ink-700)]">歌词</span>
        </div>
        <pre className="text-sm text-[var(--ink-600)] leading-relaxed whitespace-pre-wrap font-sans">
          {preview.lyrics}
        </pre>
      </div>
    </div>
    ...
    {/* 操作按钮 */}
    <div className="px-5 py-4 border-t border-[var(--paper-100)] bg-[var(--paper-50)]/50 flex items-center justify-between gap-3">
      <Button
        variant="outline"
        onClick={onEdit}
        className="flex-1 h-11 rounded-[var(--radius-sharp)] text-[var(--ink-600)] hover:text-[var(--ink-800)] hover:bg-[var(--paper-50)]"
      >
...
const SubmittingAnimation = () => (
  ...
  >
    ...
    <h3 className="text-lg font-semibold text-[var(--ink-800)] mb-2">正在提交到 Suno...</h3>
    <p className="text-sm text-[var(--ink-500)]">请稍候，正在将您的创作提交到音乐生成引擎</p>
  </motion.div>
)
    ...
    >
      ...
      <div className="flex items-center gap-6">
        <div className="relative w-20 h-20 shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <Music4 className="h-7 w-7 text-[var(--ink-600)]" />
          </div>
        </div>
        <div className="flex-1">
          <h3 className="font-semibold text-[var(--ink-800)] mb-1">{statusText}</h3>
