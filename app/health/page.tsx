      ...
      <div className="mx-auto max-w-4xl space-y-8">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-green-50 px-3 py-1 text-sm font-medium text-green-700">
            ...
            <Link
              href="/api/health"
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-green-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
            >
              查看 JSON 接口
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 rounded-lg bg-slate-100 px-3 py-2 font-mono text-sm text-slate-700">
              GET /api/health
            </div>
          </div>
            ...
            <ul className="space-y-3 text-sm leading-6 text-slate-600">
              {CHECK_ITEMS.map((item) => (
                <li key={item} className="flex gap-2">
                  <ClipboardCheck className="mt-0.5 h-4 w-4 shrink-0 text-green-700" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
