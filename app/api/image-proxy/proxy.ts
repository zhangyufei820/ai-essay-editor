...
const DEFAULT_IMAGE_GATEWAY_ORIGIN = "http://43.154.111.156:8001"
...
function getAllowedOrigins(): Set<string> {
  const origins = new Set([DEFAULT_IMAGE_GATEWAY_ORIGIN, DEFAULT_GEMINI_GATEWAY_ORIGIN])
...
function getUpstreamImageUrl(target: URL): string {
  const gatewayPairs = [
    {
      internal: process.env.DIFY_IMAGE_GATEWAY_URL,
      publicOrigins: [
        DEFAULT_IMAGE_GATEWAY_ORIGIN,
        process.env.DIFY_IMAGE_GATEWAY_PUBLIC_URL,
        process.env.NEXT_PUBLIC_DIFY_IMAGE_GATEWAY_URL,
      ],
    },
    {
      internal: process.env.GEMINI_IMAGE_GATEWAY_URL,
      publicOrigins: [
  ...
  for (const pair of gatewayPairs) {
    if (!pair.internal) continue
    try {
      const internalOrigin = new URL(pair.internal).origin
      const publicOrigins = new Set(pair.publicOrigins.filter(Boolean) as string[])

      if (publicOrigins.has(target.origin)) {
        return `${internalOrigin}${target.pathname}${target.search}`
      }
    } catch {
      // Fall back to the requested URL if the gateway env is malformed.
    }
  }
