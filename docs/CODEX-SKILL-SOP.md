...
4. 部署结束后保持本地和服务器工作区干净：本地 `git status --short --branch` 应可解释，生产 `/data/ai-essay-editor` 不应长期留下未归类的源码改动、临时文件或错误同步产物。
5. 若因线上紧急修复临时热同步文件，必须在交付总结中说明，并尽快补齐正式提交、服务器同步和工作区清理。
