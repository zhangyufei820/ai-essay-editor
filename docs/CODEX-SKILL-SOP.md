...
6. **结果验证**：检查功能、UI、流式输出、性能指标和相关测试；修复类任务必须额外模拟真实用户路径完成一次端到端验证。
7. **代码审核**：检查冗余代码、潜在 Bug、日志合理性、安全风险（SQL 注入、XSS、认证、权限）。
8. **交付总结**：说明修改点、验证结果、已知限制和后续维护建议。

## 0.2 测试通过后的自动部署与工作区卫生

当本地验证命令通过，且用户没有明确要求暂停部署时，Codex 必须继续完成生产部署和线上验证，不得停留在“本地已修复”。最低要求：

1. 本地测试、类型检查、lint 或构建中与本次变更相关的必要命令必须通过。
2. 测试通过后自动部署到生产环境，部署后检查容器状态和 `https://shenxiang.school/api/health`。
3. 部署必须确认代码真实进入生产容器，不能只看容器 healthy；必要时检查生产目录和容器内编译产物。
4. 线上验证必须包含真实用户路径模拟：从用户会进入的页面或入口开始，按真实点击、上传、提交、等待、结果展示、历史/积分等关键反馈走完一遍；不能只用健康检查、接口 curl、容器状态或日志代替。
5. 部署结束后保持本地和服务器工作区干净：本地 `git status --short --branch` 应可解释，生产 `/data/ai-essay-editor` 不应长期留下未归类的源码改动、临时文件或错误同步产物。
6. 若因线上紧急修复临时热同步文件，必须在交付总结中说明，并尽快补齐正式提交、服务器同步和工作区清理。

## 0.3 修复后的真实用户验证

每次修复线上或用户可见问题后，必须把“模拟真实用户验证”作为完成条件。最低要求：

1. 使用生产域名或本地对应页面，从真实用户会点击的入口进入，不得只调用内部 API。
2. 覆盖问题发生的完整业务链路，例如登录态、模型/工具选择、文件上传、提交、流式等待、结果渲染、积分/历史记录/下载等与本次问题相关的反馈。
3. 优先使用 Browser / Playwright / 真实浏览器会话验证页面行为；若必须用脚本或 curl 替代，必须说明无法浏览器验证的原因，并补充服务端日志或数据库证据。
4. 线上故障修复后，必须验证生产容器实际加载了新代码和新环境变量，再执行真实用户路径；不能把本地通过、构建通过或容器 healthy 当成最终结论。
5. 交付总结必须写清楚真实用户验证的入口、操作步骤、观察结果和剩余风险。

## 0.4 真实用户验证测试账号

涉及登录后真实用户路径验证时，默认使用以下 E2E 测试账号：

- 手机号：`19132896773`
- 密码：不得写入仓库、SOP、提交记录、日志或记忆文件。

密码只能从本机安全来源读取，优先级如下：

1. 环境变量：`SHENXIANG_E2E_TEST_PASSWORD`
2. 本机 secret 文件：`/Users/aixingren/.shenxiang-secrets/shenxiang-e2e.env`

本机 secret 文件建议格式：

```bash
SHENXIANG_E2E_TEST_PHONE=19132896773
SHENXIANG_E2E_TEST_PASSWORD=...
```
