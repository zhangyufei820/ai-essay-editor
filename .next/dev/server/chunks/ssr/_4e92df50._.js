...
function CheckoutFlow({ productId }) {
    ...
    const handlePayment = async (type)=>{
        // 1. 打印开始日志
        console.log(`🚀 [点击支付] 正在启动 ${type} 支付流程...`);
        // 2. 检查用户是否登录
        if (!user) {
            console.log("❌ 用户未登录，正在跳转登录页...");
            router.push(loginRedirectUrl);
            return;
        }
        setIsPaying(true);
        try {
            // 3. 获取用户ID（尝试多种字段，防止字段名不对）
            const userId = user.id || user.sub || user.user_id;
            console.log("👤 当前用户ID:", userId);
            if (!userId) {
                alert("系统错误：无法获取当前用户ID，请重新登录。");
                console.error("❌ User对象中缺少ID字段:", user);
                router.push(loginRedirectUrl);
                setIsPaying(false);
                return;
            }
            ...
            console.log("🔗 正在请求后端 API:", apiUrl);
            // 5. 发起请求
            const res = await fetch(apiUrl);
            // 6. 解析后端返回的数据
            // 注意：如果后端报错返回的不是 JSON，这里可能会报错，所以要小心
            let data;
            try {
                data = await res.json();
                console.log("📦 后端返回的完整数据:", data); // 【重点】看控制台这里打印了什么
            } catch (jsonError) {
                console.error("❌ 解析 JSON 失败，可能后端返回了 HTML 报错页面", jsonError);
                throw new Error("后端服务异常 (JSON 解析失败)");
            }
            if (!res.ok) {
                throw new Error(data.error || `请求失败，状态码: ${res.status}`);
            }
            // 7. 获取跳转链接 (兼容 url 或 pay_url 或 link 字段)
            const redirectUrl = data.url || data.pay_url || data.link;
            if (redirectUrl) {
                console.log("✅ 成功！正在跳转到:", redirectUrl);
                // 执行跳转
                window.location.href = redirectUrl;
            } else {
                console.error("❌ 数据中没找到跳转链接 (url 字段为空)", data);
                throw new Error("支付接口未返回有效的支付链接");
            }
        } catch (e) {
            console.error("💥 支付流程发生异常:", e);
            // 弹窗告诉用户具体错在哪
            alert(`支付启动失败：\n${e.message}`);
            setIsPaying(false); // 只有出错时才取消 loading，让用户能重试
        }
    };
    // Loading 界面
    if (loading) {
...
//# sourceMappingURL=_4e92df50._.js.map