...
"[project]/app/api/user/credits/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        // 查询积分
        const { data: creditData, error } = await supabaseAdmin.from('user_credits').select('credits, is_pro').eq('user_id', userId).maybeSingle();
        if (error) {
        ...
        if (!creditData) {
            ...
            const { data: newData, error: insertError } = await supabaseAdmin.from('user_credits').upsert({
                user_id: userId,
                credits: 1000,
                is_pro: false
            }).select('credits, is_pro').single();
            if (insertError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__02df6f95._.js.map