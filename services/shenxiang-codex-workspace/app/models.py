...
class ModelRoleConfig(BaseModel):
    chat_main: str = Field(default="", max_length=120)
    small_fast: str = Field(default="", max_length=120)
    web_search: str = Field(default="", max_length=120)
    image_generation: str = Field(default="", max_length=120)
    video_generation: str = Field(default="", max_length=120)
    code_review: str = Field(default="", max_length=120)


class WorkspaceFile(BaseModel):
    path: str = Field(..., min_length=1, max_length=180)
    content: str = Field(..., max_length=9000000)


class WorkspaceRunRequest(BaseModel):
    model_config = ConfigDict(populate_by_name=True)

    user_query: str = Field(..., min_length=1, max_length=50000)
    skill_name: str = Field(default="codex_workspace", min_length=1, max_length=80, pattern=r"^[a-zA-Z0-9_-]+$")
    model_role: ModelRole = "chat_main"
    model_roles: ModelRoleConfig = Field(default_factory=ModelRoleConfig, alias="model_config")
    task_type: str = Field(default="codex_workspace", max_length=120)
...
class UserSkillCreateRequest(CreateSkillRequest):
    install_immediately: bool = True


class SkillMarkdownImportRequest(BaseModel):
