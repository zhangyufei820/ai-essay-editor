...
def codex_connection_page() -> HTMLResponse:
    ...
    html = html.replace(
        "</main><script>",
        """<section class="guide"><h2>接下来怎么填</h2><ol><li>复制上面的连接码。</li><li>打开 Codex 的设置，进入“插件 → MCP → 连接至自定义 MCP”。</li><li>名称填写“星人工具”，类型选择“流式 HTTP”。</li><li>URL 填写 <code>https://api.aiphui.top/codex/mcp</code>。</li><li>在“标头”新增一行：键填写 <code>Authorization</code>；值先填写 <code>Bearer </code>，再粘贴连接码，然后保存。</li></ol><p class="hint">不要填写 Bearer 令牌环境变量，也不要填写其他标头。</p><p><a href="/codex/docs/agent-connect" target="_blank" rel="noopener noreferrer">查看完整图文说明</a></p></section></main><script>""",
    )
    html = html.replace("</style>", ".guide{margin-top:28px;padding-top:20px;border-top:1px solid #dce2ef;line-height:1.7}.guide h2{font-size:19px;margin:0 0 10px}.guide ol{padding-left:22px}.guide code{background:#f4f7ff;padding:2px 5px;border-radius:4px;word-break:break-all}.guide a{color:#2558d9}</style>")
    return HTMLResponse(html, headers={"Cache-Control": "no-store", "X-Frame-Options": "DENY"})
