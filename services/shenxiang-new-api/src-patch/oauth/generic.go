...
func (p *GenericOAuthProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken request: auth_style=%d", p.config.Slug, authStyle)
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken request failed", p.config.Slug))
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken response read failed", p.config.Slug))
		return nil, err
	}
	...
	if err := common.Unmarshal(body, &tokenResponse); err != nil {
		...
		if parseErr != nil {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken response parse failed", p.config.Slug))
			return nil, err
		}
	...
	if tokenResponse.Error != "" {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken provider rejected request", p.config.Slug))
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken succeeded", p.config.Slug)
...
func (p *GenericOAuthProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	res, err := client.Do(req)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] GetUserInfo request failed", p.config.Slug))
		return nil, NewOAuthErrorWithRaw(i18n.MsgOAuthConnectFailed, map[string]any{"Provider": p.config.Name}, err.Error())
	}
	defer res.Body.Close()
	...
	body, err := io.ReadAll(res.Body)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] GetUserInfo response read failed", p.config.Slug))
		return nil, err
	}
	...
	if policyRaw != "" {
		...
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] invalid access policy", p.config.Slug))
