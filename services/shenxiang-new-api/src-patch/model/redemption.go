...
type RedemptionResult struct {
	...
	ImageBenefit       *ImageBenefitTokenResult `json:"image_benefit,omitempty"`
}
...
func Redeem(key string, userId int) (*RedemptionResult, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if IsImageBenefitRedemption(redemption) {
			token, err := CreateImageBenefitTokenTx(tx, userId)
			if err != nil {
				return err
			}
			result = &RedemptionResult{
				Type:         "image_benefit",
				Quota:        0,
				ImageBenefit: ImageBenefitTokenResultFromToken(token),
			}
		} else if redemption.PlanId > 0 {
			plan, err := getSubscriptionPlanByIdTx(tx, redemption.PlanId)
			if err != nil {
				return err
			}
			if !plan.Enabled {
				return errors.New("订阅方案未启用")
			}
	...
	if result.Type == "image_benefit" {
		RecordLog(userId, LogTypeTopup, fmt.Sprintf("通过兑换码开通%s，兑换码ID %d", ImageBenefitRedemptionName, redemption.Id))
	} else if result.Type == "subscription" {
		RecordLog(userId, LogTypeTopup, fmt.Sprintf("通过兑换码开通套餐 %s，兑换码ID %d，订阅ID %d", result.PlanTitle, redemption.Id, result.UserSubscriptionId))
	} else {
