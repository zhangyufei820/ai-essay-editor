...
type SubscriptionPlan struct {
	...
	// MonthlyAmountTotal is an optional lifetime/month-cycle hard cap.
	// For daily-reset plans, TotalAmount remains the reset window quota.
	MonthlyAmountTotal int64 `json:"monthly_amount_total" gorm:"type:bigint;not null;default:0"`

	// ConcurrencyLimit limits simultaneous subscription-funded relay requests.
...
func HasActiveUserSubscriptionPlanTitle(userId int, planTitle string) (bool, error) {
	if userId <= 0 {
		return false, errors.New("invalid userId")
	}
	planTitle = strings.TrimSpace(planTitle)
	if planTitle == "" {
		return false, errors.New("planTitle is empty")
	}
	now := common.GetTimestamp()
	var count int64
	if err := DB.Model(&UserSubscription{}).
		Joins("JOIN subscription_plans ON subscription_plans.id = user_subscriptions.plan_id").
		Where("user_subscriptions.user_id = ? AND user_subscriptions.status = ? AND user_subscriptions.end_time > ? AND subscription_plans.title = ?",
			userId, "active", now, planTitle).
		Count(&count).Error; err != nil {
		return false, err
	}
	return count > 0, nil
...
func PreConsumeUserSubscription(requestId string, userId int, modelName string, quotaType int, amount int64) (*SubscriptionPreConsumeResult, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		dailyHardBlocked := false
		monthlyHardBlocked := false
		...
		for _, candidate := range subs {
			...
			hardLimitPlan := sub.MonthlyAmountTotal > 0 || sub.ConcurrencyLimit > 0 || plan.MonthlyAmountTotal > 0 || plan.ConcurrencyLimit > 0
...
func PostConsumeUserSubscriptionDelta(userSubscriptionId int, delta int64) error {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		newMonthlyUsed := sub.MonthlyAmountUsed
		if sub.MonthlyAmountTotal > 0 {
			newMonthlyUsed = sub.MonthlyAmountUsed + delta
			if newMonthlyUsed < 0 {
				newMonthlyUsed = 0
			}
			if newMonthlyUsed > sub.MonthlyAmountTotal {
				return &SubscriptionLimitError{Reason: "monthly"}
			}
		}
