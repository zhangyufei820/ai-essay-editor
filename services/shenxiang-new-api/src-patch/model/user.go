...
var ErrInsufficientQuota = errors.New("insufficient user quota")
...
// normalized email inside tx, so a check-then-write sequence cannot race.
...
func IsEmailTakenByOtherUser(email string, excludeUserId int) (bool, error) {
	...
	var count int64
	if err := emailQuery(DB, email).Where("id <> ?", excludeUserId).Count(&count).Error; err != nil {
		return false, err
	}
	return count > 0, nil
}
...
func TryDecreaseUserQuota(id int, quota int) error {
	if id <= 0 {
	...
	if err := invalidateUserCache(id); err != nil {
		common.SysLog("failed to invalidate user cache after conditional quota decrease: " + err.Error())
	}
	return nil
}
...
func UpdateUsageStats(userId int, channelId int, quota int) {
	if common.BatchUpdateEnabled {
	...
	if err != nil {
		common.SysLog(fmt.Sprintf("failed to update usage stats: user_id=%d, channel_id=%d, delta_quota=%d, error=%v", userId, channelId, quota, err))
	}
}
