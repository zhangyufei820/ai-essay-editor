...
func (e *NewAPIError) PublicMessage() string {
	...
	if strings.Contains(lowerMsg, "no access to model") ||
		...
		strings.Contains(msg, "无权访问模型") ||
		strings.Contains(msg, "无权访问任何模型") ||
		strings.Contains(msg, "無權存取模型") ||
		strings.Contains(msg, "無權存取任何模型") ||
		e.errorCode == ErrorCodeAccessDenied {
		return "当前账号暂未开通该模型，请联系管理员或切换模型。"
	}

	switch e.errorCode {
	case ErrorCodeInvalidRequest, ErrorCodeBadRequestBody, ErrorCodeConvertRequestFailed:
	...
	case ErrorCodeGetChannelFailed, ErrorCodeDoRequestFailed, ErrorCodeBadResponseStatusCode,
		...
		ErrorCodeChannelNoAvailableKey, ErrorCodeChannelParamOverrideInvalid,
		ErrorCodeChannelHeaderOverrideInvalid, ErrorCodeChannelModelMappedError,
		ErrorCodeChannelAwsClientError, ErrorCodeChannelInvalidKey,
		ErrorCodeChannelResponseTimeExceeded:
		return "模型服务暂时不可用，请稍后重试。"
	}

	if strings.Contains(code, "channel") ||
		strings.Contains(code, "upstream") ||
		ContainsProviderDisclosure(lowerMsg) ||
		ContainsProviderDisclosure(code) {
		return "模型服务暂时不可用，请稍后重试。"
	}
...
func publicErrorCode(code any) any {
	codeStr := strings.ToLower(fmt.Sprintf("%v", code))
	if code == nil || codeStr == "" || codeStr == "<nil>" {
		return "service_unavailable"
	}
	if ContainsProviderDisclosure(codeStr) ||
		strings.Contains(codeStr, "bad_response") ||
		strings.Contains(codeStr, "do_request") ||
		strings.Contains(codeStr, "get_channel") {
		return "service_unavailable"
	}
	return code
}
