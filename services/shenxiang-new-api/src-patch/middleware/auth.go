...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	if service.IsMonthlyCardTokenName(token.Name) {
		c.Set("monthly_card_token", true)
	}
	if model.IsImageBenefitTokenName(token.Name) {
		c.Set("image_benefit_token", true)
	}
	c.Set("token_unlimited_quota", token.UnlimitedQuota)
	c.Set("token_daily_quota", token.DailyQuota)
	c.Set("token_monthly_quota", token.MonthlyQuota)
