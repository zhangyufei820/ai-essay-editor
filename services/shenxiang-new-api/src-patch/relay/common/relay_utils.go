...
func GetFullRequestURL(baseURL string, requestURL string, channelType int) string {
	if strings.HasSuffix(strings.TrimRight(baseURL, "/"), "/v1") && strings.HasPrefix(requestURL, "/v1/") && !strings.HasPrefix(baseURL, "https://gateway.ai.cloudflare.com") {
		return fmt.Sprintf("%s%s", strings.TrimRight(baseURL, "/"), strings.TrimPrefix(requestURL, "/v1"))
	}
...
// as a billing multiplier, so unbounded values can poison quota calculation.
