...
const TaskSubmitDispatchedContextKey = "task_submit_dispatched"

var realtimeChannelLookup = model.GetChannelById

var realtimePollingAdaptorFactory = func(channelType int) service.TaskPollingAdaptor {
	adaptor := GetTaskAdaptor(constant.TaskPlatform(strconv.Itoa(channelType)))
	if adaptor == nil {
		return nil
	}
	pollingAdaptor, _ := adaptor.(service.TaskPollingAdaptor)
	return pollingAdaptor
}

func bufferTaskSubmitResponse(resp *http.Response) ([]byte, error) {
	if resp == nil {
		return nil, errors.New("task upstream response is nil")
	}
	if resp.Body == nil {
		return nil, errors.New("task upstream response body is nil")
	}
	body := resp.Body
	defer body.Close()
	responseBody, err := common.ReadAllCapped(body)
	if err != nil {
		return nil, err
	}
...
func sanitizeDispatchedTaskError(taskErr *dto.TaskError) *dto.TaskError {
	if taskErr == nil {
		return nil
	}
...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	// 9. 发送请求。进入 DoRequest 后结果即可能不确定，调用方不得自动重投。
	c.Set(TaskSubmitDispatchedContextKey, true)
	resp, err := adaptor.DoRequest(c, info, requestBody)
	if err != nil {
		return nil, sanitizeDispatchedTaskError(
			service.TaskErrorWrapper(err, "do_request_failed", http.StatusInternalServerError),
		)
	}
	responseBody, err := bufferTaskSubmitResponse(resp)
	if err != nil {
		return nil, sanitizeDispatchedTaskError(
			service.TaskErrorWrapper(err, "read_response_body_failed", http.StatusBadGateway),
		)
	}
	...
	if taskErr != nil {
		return nil, sanitizeDispatchedTaskError(taskErr)
	}
...
func tryRealtimeFetch(ctx context.Context, task *model.Task, isOpenAIVideoAPI bool) []byte {
	...
	channelModel, err := realtimeChannelLookup(task.ChannelId, true)
	if err != nil {
		return nil
	}
	...
	pollingAdaptor := realtimePollingAdaptorFactory(channelModel.Type)
	if pollingAdaptor == nil {
		return nil
	}
	...
	defer requestCancel()
	resp, err := pollingAdaptor.FetchTaskWithContext(requestCtx, baseURL, key, map[string]any{
	...
	ti, err := pollingAdaptor.ParseTaskResult(body)
	if err != nil || ti == nil {
		return nil
	}
	...
	if task.Status == model.TaskStatusFailure {
		task.FailReason = service.SanitizeAsyncTaskFailure(ti.Reason)
	}
	...
	if !snap.Equal(task.Snapshot()) {
		...
		if !won {
			reloaded, exists, reloadErr := model.GetByOnlyTaskIdWithContext(ctx, task.TaskID)
