...
func ResponseChunkData(c *gin.Context, resp dto.ResponsesStreamResponse, data string) error {
	...
	if written, err := c.Writer.WriteString(eventData); err != nil {
		return fmt.Errorf("write responses stream event failed: %w", err)
	} else if written != len(eventData) {
		return fmt.Errorf("write responses stream event failed: %w", io.ErrShortWrite)
	}
	responseData := fmt.Sprintf("data: %s\n\n", strings.ReplaceAll(data, "\r", "\\r"))
	if written, err := c.Writer.WriteString(responseData); err != nil {
		return fmt.Errorf("write responses stream data failed: %w", err)
	} else if written != len(responseData) {
		return fmt.Errorf("write responses stream data failed: %w", io.ErrShortWrite)
	}
