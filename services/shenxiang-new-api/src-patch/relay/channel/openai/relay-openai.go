...
func preConsumeUsage(ctx *gin.Context, info *relaycommon.RelayInfo, usage *dto.RealtimeUsage, totalUsage *dto.RealtimeUsage) error {
	...
	service.MergeRealtimeUpstreamCost(totalUsage, usage)
	// clear usage
...
func applyUsageCostPostProcessing(usage *dto.Usage, responseBody []byte) {
	...
	var payload struct {
		...
		CostCNY      any                   `json:"cost_cny,omitempty"`
		CostCurrency string                `json:"cost_currency,omitempty"`
		Currency     string                `json:"currency,omitempty"`
		CostDetails  *dto.UsageCostDetails `json:"cost_details,omitempty"`
		Billing      *dto.UsageBilling     `json:"billing,omitempty"`
		Usage        dto.Usage             `json:"usage"`
	}
	...
	if usage.Cost == nil {
		if payload.Usage.Cost != nil {
			usage.Cost = payload.Usage.Cost
		} else {
			usage.Cost = payload.Cost
		}
	}
	if usage.TotalCost == nil {
		if payload.Usage.TotalCost != nil {
			usage.TotalCost = payload.Usage.TotalCost
		} else {
			usage.TotalCost = payload.TotalCost
		}
	}
	if usage.CostUSD == nil {
		if payload.Usage.CostUSD != nil {
			...
			usage.CostUSD = payload.CostUSD
		}
	}
	if usage.CostCNY == nil {
		if payload.Usage.CostCNY != nil {
			usage.CostCNY = payload.Usage.CostCNY
		} else {
			usage.CostCNY = payload.CostCNY
		}
	}
	if usage.CostCurrency == "" {
		if payload.Usage.CostCurrency != "" {
			usage.CostCurrency = payload.Usage.CostCurrency
		} else {
			usage.CostCurrency = payload.CostCurrency
		}
	}
	if usage.Currency == "" {
		if payload.Usage.Currency != "" {
			usage.Currency = payload.Usage.Currency
		} else {
			usage.Currency = payload.Currency
		}
	}
	if usage.CostDetails == nil {
		if payload.Usage.CostDetails != nil {
			usage.CostDetails = payload.Usage.CostDetails
		} else {
			usage.CostDetails = payload.CostDetails
		}
	}
	if usage.Billing == nil {
		if payload.Usage.Billing != nil {
			usage.Billing = payload.Usage.Billing
		} else {
			usage.Billing = payload.Billing
		}
	}
}
