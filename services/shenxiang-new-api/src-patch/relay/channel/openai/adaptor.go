...
func (a *Adaptor) ConvertImageRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (any, error) {
	...
	case relayconstant.RelayModeImagesEdits:
		if shouldRouteMoonapixImageEditViaGeneration(info) {
			return convertMoonapixImageEditToGeneration(c, info, request)
		}
...
func shouldRouteMoonapixImageEditViaGeneration(info *relaycommon.RelayInfo) bool {
	if info == nil || info.RelayMode != relayconstant.RelayModeImagesEdits {
		return false
	}
	if !strings.Contains(strings.ToLower(info.ChannelBaseUrl), "moonapix.com") {
		return false
	}

	switch strings.ToLower(info.OriginModelName) {
	case "banana-2", "gemini-3-pro-image-preview":
		return true
	}
	switch strings.ToLower(info.UpstreamModelName) {
...
func convertMoonapixImageEditToGeneration(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (dto.ImageRequest, error) {
	if !isJSONRequest(c) {
		imageDataURL, err := readFirstMultipartImageAsDataURL(c)
		if err != nil {
			return request, err
		}
		imageJSON, err := common.Marshal(imageDataURL)
		if err != nil {
			return request, fmt.Errorf("failed to marshal image data url: %w", err)
		}
		request.Image = imageJSON
	}
...
func readFirstMultipartImageAsDataURL(c *gin.Context) (string, error) {
	mf := c.Request.MultipartForm
	if mf == nil {
		if _, err := c.MultipartForm(); err != nil {
	...
	if mf == nil || mf.File == nil {
		return "", errors.New("no multipart form data found")
	}

	fileHeader, err := firstImageFileHeader(mf)
	if err != nil {
		return "", err
	}
...
func firstImageFileHeader(mf *multipart.Form) (*multipart.FileHeader, error) {
