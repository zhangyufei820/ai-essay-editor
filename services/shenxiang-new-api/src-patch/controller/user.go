...
func GetSelf(c *gin.Context) {
	...
	textPricingGroup, pricingErr := model.ResolveUserTextPricingGroup(id)
	if pricingErr != nil {
		logger.LogError(c, "failed to resolve text pricing preference: "+pricingErr.Error())
		textPricingGroup = model.TextPricingGroupDefault
	}
	...
	responseData := map[string]interface{}{
		...
		"text_pricing_group": textPricingGroup,
...
func GetUserModels(c *gin.Context) {
	...
	if group != "" {
		...
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"message": "",
			"data":    normalizeUserVisibleModels(id, model.GetGroupEnabledModels(group)),
		})
		return
	}
