...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	displayModelName := PublicImageModelDisplayName(modelName, ctx.GetString("public_image_model_alias"))
	...
	quota, upstreamCostBilling := ApplyRealtimeUpstreamCostBilling(relayInfo, usage, quota)
	if upstreamCostLog := FormatUpstreamCostBillingLog(upstreamCostBilling); upstreamCostLog != "" {
		if extraContent != "" {
			extraContent += ", "
		}
		extraContent += upstreamCostLog
	}
	...
	if totalTokens == 0 && (upstreamCostBilling == nil || !upstreamCostBilling.Applied) {
		// in this case, must be some error happened
		// we cannot just return, because we may have to return the pre-consumed quota
		quota = 0
		logContent += "（可能是上游超时）"
		logger.LogError(ctx, fmt.Sprintf("total tokens is 0, cannot consume quota, userId %d, channelId %d, "+
			"tokenId %d, model %s， pre-consumed quota %d", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, displayModelName, relayInfo.FinalPreConsumedQuota))
	} else {
		model.UpdateUsageStats(relayInfo.UserId, relayInfo.ChannelId, quota)
	}
	...
	if tieredResult != nil && (upstreamCostBilling == nil || !upstreamCostBilling.Applied) {
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	tokenName := ctx.GetString("token_name")
	displayModelName := PublicImageModelDisplayName(relayInfo.OriginModelName, ctx.GetString("public_image_model_alias"))
	...
	quota, upstreamCostBilling := ApplyUpstreamCostBilling(relayInfo, usage, quota)
	if upstreamCostLog := FormatUpstreamCostBillingLog(upstreamCostBilling); upstreamCostLog != "" {
		if extraContent != "" {
			extraContent += ", "
		}
...
func PreConsumeTokenQuota(relayInfo *relaycommon.RelayInfo, quota int) error {
	...
	if err != nil {
		return fmt.Errorf("token quota is not enough or quota update failed, need quota: %s", logger.FormatQuota(quota))
	}
	return nil
}
