...
func decodeToolCallMaxIndexOffset(value int) (maxOffset int, absoluteIndexes bool) {
	if value < 0 {
		return -value - 1, true
	}
	return value, false
...
func encodeToolCallMaxIndexOffset(maxOffset int, absoluteIndexes bool) int {
	if absoluteIndexes {
		return -maxOffset - 1
	}
	return maxOffset
...
func StreamResponseOpenAI2Claude(openAIResponse *dto.ChatCompletionsStreamResponse, info *relaycommon.RelayInfo) []*dto.ClaudeResponse {
	...
	stopOpenBlocks := func() {
		...
		case relaycommon.LastMessageTypeTools:
			...
			maxOffset, _ := decodeToolCallMaxIndexOffset(info.ClaudeConvertInfo.ToolCallMaxIndexOffset)
	...
	stopOpenBlocksAndAdvance := func() {
		...
		case relaycommon.LastMessageTypeTools:
			maxOffset, _ := decodeToolCallMaxIndexOffset(info.ClaudeConvertInfo.ToolCallMaxIndexOffset)
			info.ClaudeConvertInfo.Index = info.ClaudeConvertInfo.ToolCallBaseIndex + maxOffset + 1
	...
	if len(openAIResponse.Choices) == 0 {
		...
		if len(chosenChoice.Delta.ToolCalls) > 0 {
			...
			if info.ClaudeConvertInfo.LastMessagesType != relaycommon.LastMessageTypeTools {
				...
				info.ClaudeConvertInfo.ToolCallMaxIndexOffset = 0
				if firstIndex := toolCalls[0].Index; firstIndex != nil &&
					info.ClaudeConvertInfo.ToolCallBaseIndex > 0 &&
					*firstIndex >= info.ClaudeConvertInfo.ToolCallBaseIndex {
					absoluteIndexes = true
				}
			} else {
				_, absoluteIndexes = decodeToolCallMaxIndexOffset(info.ClaudeConvertInfo.ToolCallMaxIndexOffset)
			}
			info.ClaudeConvertInfo.LastMessagesType = relaycommon.LastMessageTypeTools
			base := info.ClaudeConvertInfo.ToolCallBaseIndex
			maxOffset, _ := decodeToolCallMaxIndexOffset(info.ClaudeConvertInfo.ToolCallMaxIndexOffset)
			...
			for i, toolCall := range toolCalls {
				...
				if toolCall.Index != nil {
					if absoluteIndexes && *toolCall.Index >= base {
						blockIndex = *toolCall.Index
					} else {
						blockIndex = base + *toolCall.Index
					}
				}
				offset := blockIndex - base
				if offset > maxOffset {
					maxOffset = offset
				}
			...
			info.ClaudeConvertInfo.ToolCallMaxIndexOffset = encodeToolCallMaxIndexOffset(maxOffset, absoluteIndexes)
