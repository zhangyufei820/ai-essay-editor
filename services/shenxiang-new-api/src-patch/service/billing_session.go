	...
	if err := s.funding.PreConsume(effectiveQuota); err != nil {
		...
		if errors.Is(err, model.ErrUserQuotaInsufficient) {
			return types.NewErrorWithStatusCode(
				fmt.Errorf("用户额度不足, 需要预扣额度: %s", logger.FormatQuota(effectiveQuota)),
...
func (s *BillingSession) reserveFunding(delta int) error {
	...
	case *WalletFunding:
		if err := model.DecreaseUserQuotaIfEnough(funding.userId, delta); err != nil {
			if errors.Is(err, model.ErrUserQuotaInsufficient) {
				return types.NewErrorWithStatusCode(
					fmt.Errorf("用户额度不足, 需要补充预扣额度: %s", logger.FormatQuota(delta)),
					types.ErrorCodeInsufficientUserQuota,
					http.StatusForbidden,
					types.ErrOptionWithSkipRetry(),
					types.ErrOptionWithNoRecordErrorLog(),
...
func NewBillingSession(c *gin.Context, relayInfo *relaycommon.RelayInfo, preConsumedQuota int) (*BillingSession, *types.NewAPIError) {
	...
	trySubscription := func() (*BillingSession, *types.NewAPIError) {
		...
		if isMonthlyCardToken {
			skipTokenQuota = true
		} else if hasMonthlyCard && canUseSubscriptionFundingForModel(hasMonthlyCard, relayInfo.OriginModelName) {
			skipTokenQuota = true
		}
		session := &BillingSession{
			relayInfo:      relayInfo,
	...
	case "monthly_card_and_wallet":
		...
		if hasMonthlyCard && canUseSubscriptionFundingForModel(hasMonthlyCard, relayInfo.OriginModelName) {
