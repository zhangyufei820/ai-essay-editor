...
function officialUsdFromQuota(quota) {
  const usd = Number(quota || 0) / QUOTA_PER_OFFICIAL_USD;
...
function isValueMonthlyCard(plan) {
  return (
              ...
              <>
                ...
                <div className='max-h-64 overflow-y-auto pr-1 semi-table-body'>
                  {allSubscriptions.map((sub, subIndex) => {
                    ...
                    const monthlyAmountTotal = Number(
                      subscription?.monthly_amount_total || 0,
                    );
                    const monthlyAmountUsed = Number(
                    ...
                    const monthlyRemainAmount =
                      monthlyAmountTotal > 0
                        ? Math.max(0, monthlyAmountTotal - monthlyAmountUsed)
                        : 0;
                    const planTitle =
                    ...
                    const monthlyCard =
                      monthlyAmountTotal > 0 || isValueMonthlyCard({ title: planTitle });
                    const now = Date.now() / 1000;
                            ...
                            <div className='space-y-2'>
                              <div>
                                ...
                                <span>
                                  ...
                                  {' · '}
                                  {t('剩余')} $
                                  {officialUsdFromQuota(remainAmount)}
                                  {' · '}
                                  {t('已用')} {usagePercent}%
                                </span>
                              </div>
                              ...
                              <div>
                                {t('月总额度')}:{' '}
                                <span>
                                  ${officialUsdFromQuota(monthlyAmountUsed)}/$
                                  {officialUsdFromQuota(monthlyAmountTotal)}
                                  {' · '}
                                  {t('剩余')} $
                                  {officialUsdFromQuota(monthlyRemainAmount)}
                                  {' · '}
                                  {t('已用')} {monthlyUsagePercent}%
                                </span>
                              </div>
                              {monthlyAmountTotal > 0 && (
                                <div className='h-1.5 rounded-full bg-gray-200 overflow-hidden'>
          ...
          {plans.length > 0 ? (
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-5 w-full px-1'>
              {plans.map((p, index) => {
                ...
                const displaySymbol = monthlyCard ? '¥' : symbol;
                const displayPrice = monthlyCard
                  ? '500'
                ...
                const planBenefits = monthlyCard
                  ? [
                      {
                        label: `${t('每日额度')}: $${officialUsdFromQuota(totalAmount)} ${t('官网等值额度')}`,
                      },
                      {
                        label: `${t('月总额度')}: $${officialUsdFromQuota(monthlyAmount)}`,
                      },
                      {
                        label: `${t('有效期')}: ${formatSubscriptionDuration(plan, t)}`,
                      },
                      { label: `${t('并发')}: ${concurrencyLimit || 5}` },
                      { label: t('适合 Codex 重度开发使用') },
                    ]
                  : [
                      {
                        label: `${t('有效期')}: ${formatSubscriptionDuration(plan, t)}`,
                    ...
                    <div className='p-4 h-full flex flex-col'>
                      ...
                      <div className='mt-auto'>
                        ...
                        {(() => {
                          ...
                          const useCodeOnly = monthlyCard && !enableOnlineTopUp;
                          const tip = reached
