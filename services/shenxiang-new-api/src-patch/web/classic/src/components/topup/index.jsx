...
import ImageBenefitCard from './ImageBenefitCard';
...
const TopUp = () => {
  ...
  const [imageBenefit, setImageBenefit] = useState(null);
  const [imageBenefitLoading, setImageBenefitLoading] = useState(false);
  ...
  const topUp = async () => {
    ...
    try {
      ...
      if (success) {
        const redemption =
          typeof data === 'number' ? { type: 'quota', quota: data } : data || {};
        if (redemption.type === 'subscription') {
          showSuccess(t('兑换成功！'));
          Modal.success({
            title: t('兑换成功！'),
            content: t('已开通套餐：') + (redemption.plan_title || t('套餐')),
            centered: true,
          });
          await getUserQuota();
        ...
        if (redemption.type === 'image_benefit') {
          showSuccess(t('兑换成功！'));
          Modal.success({
            title: t('兑换成功！'),
            content: t('已开通 1.5K 图像福利包，7 天内可成功生成 300 张。'),
            centered: true,
          });
          setImageBenefit(redemption.image_benefit || null);
          await getImageBenefit();
          setRedemptionCode('');
          return;
        }
        const quotaAdded = parseInt(redemption.quota || 0);
        showSuccess(t('兑换成功！'));
        Modal.success({
          title: t('兑换成功！'),
          content: t('成功兑换额度：') + renderQuota(quotaAdded),
          centered: true,
        });
        if (userState.user) {
          const updatedUser = {
    ...
    try {
      ...
      if (res !== undefined) {
        ...
        if (message === 'success') {
          if (payWay === 'stripe') {
            // Stripe 支付回调处理
            if (!openPaymentPage(data.pay_link)) {
              showError(t('支付跳转地址不安全'));
            }
          } else {
            // 普通支付表单提交
            if (!submitPaymentForm({ url: res.data.url, params: data })) {
              showError(t('支付跳转地址不安全'));
            }
          }
        } else {
    ...
    try {
      ...
      if (res !== undefined) {
        ...
        if (message === 'success' && data?.payment_url) {
          if (!openPaymentPage(data.payment_url)) {
            showError(t('支付跳转地址不安全'));
          }
        } else {
    ...
    try {
      ...
      if (res !== undefined) {
        ...
        if (message === 'success') {
          ...
          if (checkoutUrl && navigateToPaymentPage(checkoutUrl)) {
  ...
  const processCreemCallback = (data) => {
    // 与 Stripe 保持一致的实现方式
    if (!openPaymentPage(data?.checkout_url)) {
      showError(t('支付跳转地址不安全'));
    }
  };
  ...
  const getImageBenefit = async () => {
    setImageBenefitLoading(true);
    try {
      const res = await API.get('/api/user/image-benefit');
      if (res.data?.success) {
        setImageBenefit(res.data.data || null);
      }
    } catch (e) {
      // ignore
    } finally {
      setImageBenefitLoading(false);
    }
  };
  ...
  const handleBenefitCopy = async (value) => {
    if (await copy(value)) {
    ...
    <div className='w-full max-w-7xl mx-auto relative min-h-screen lg:min-h-0 mt-[60px] px-2'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <RechargeCard
          ...
          onRedeemFocus={() => {
            const input = document.querySelector('input[placeholder="请输入兑换码"]');
        ...
        <ImageBenefitCard
          ...
          loading={imageBenefitLoading}
          onRefresh={getImageBenefit}
          onCopy={handleBenefitCopy}
          t={t}
        />
        <InvitationCard
          t={t}
