...
function formatPlanPaymentAmount(plan) {
  const price = Number(plan?.price_amount || 0);
  const currency = String(plan?.currency || 'CNY').trim().toUpperCase();
...
const SubscriptionPurchaseModal = ({
  ...
  const displayPrice = plan ? formatPlanPaymentAmount(plan) : '¥0';
        ...
        <div className='space-y-4 pb-10'>
          ...
          {hasAnyPayment ? (
            ...
            <Banner
              type='info'
              description={t(
                '管理员未开启在线支付功能，请添加管理员个人微信号：AIXINGRENHUIHUI。',
              )}
              className='!rounded-xl'
              closeIcon={null}
            />
