...
function formatPlanOptionLabel(plan, t) {
  if (!plan) return '';
  if (isValueMonthlyCard(plan)) {
    return `${plan.title || t('月卡套餐')} · ¥500 · ${t('30天有效')} · ${t('每日')} $${officialUsdFromQuota(plan.total_amount)} · ${t('月总')} $${officialUsdFromQuota(plan.monthly_amount_total)}`;
  }
  return `${plan.title || ''} (${convertUSDToCurrency(
              ...
              {monthlyCard ? (
                <>
                  <div className='text-gray-600'>
                    {t('日额度')}: ${officialUsdFromQuota(used)} / $
                    {officialUsdFromQuota(total)}
                  </div>
                  <div className='text-gray-500'>
                    {t('月总')}: ${officialUsdFromQuota(monthlyUsed)} / $
