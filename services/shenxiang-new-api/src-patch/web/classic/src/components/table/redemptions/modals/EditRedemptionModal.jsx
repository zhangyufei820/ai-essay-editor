...
const EditRedemptionModal = (props) => {
  ...
  useEffect(() => {
    ...
    API.get('/api/subscription/admin/plans').then((res) => {
      if (res.data?.success) {
        setPlans((res.data.data || []).filter((item) => item.plan?.enabled));
      }
    });
  }, [props.visiable]);
  ...
  const submit = async (values) => {
    ...
    if (localInputs.benefit_type !== 'subscription' && localInputs.quota <= 0) {
      showError(t('请输入金额'));
      setLoading(false);
      return;
    }
                    ...
                    <Col span={24}>
                      <Form.Select
                        ...
                        optionList={[
                          ...
                          { label: t('月卡兑换码'), value: 'subscription' },
                        ]}
                      />
                    </Col>
