...
const CLAUDE_DISPLAY_GROUP_ORDER = [
  'kiro',
  'kiro-stable',
  'ccmax-terminal',
  'claude-external',
...
const ModelPricingTable = ({
  ...
  const groupLabel = (group) => usableGroup?.[group] || group;
  const renderGroupLabel = (group) => (
    <>
      {groupLabel(group)}
      {!CLAUDE_DISPLAY_GROUPS.has(group) && t('分组')}
    </>
  );

  const renderGroupPriceTable = () => {
    // 仅展示模型可用的分组：模型 enable_groups 与用户可用分组的交集
    const availableGroups = modelEnableGroups
      .filter((g) => g !== '')
      .filter((g) => g !== 'auto')
      .filter((g) =>
        Object.prototype.hasOwnProperty.call(usableGroup || {}, g),
      );
    ...
    const tableData = availableGroups.map((group) => {
      ...
      // 获取分组倍率
      const groupRatioValue = priceData?.usedGroupRatio ?? 1;
