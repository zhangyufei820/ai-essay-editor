...
const PricingCardView = ({
  ...
  const getModelIcon = (model) => {
    ...
    const avatarText = getPricingModelDisplayName(model).slice(0, 2).toUpperCase();
    return (
      <div className={CARD_STYLES.container}>
        <Avatar
          size='large'
  ...
  const renderTags = (record) => {
    ...
    const billingLabel = getPricingBillingLabel(record, t);
    if (billingLabel === t('按次计费') || billingLabel === '按张计费') {
      billingTag = (
        <Tag key='billing' shape='circle' color='teal' size='small'>
          {billingLabel}
        </Tag>
      );
    } else if (billingLabel === t('按量计费')) {
      billingTag = (
