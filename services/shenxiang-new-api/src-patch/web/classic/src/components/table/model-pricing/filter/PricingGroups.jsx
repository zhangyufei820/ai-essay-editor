...
const PricingGroups = ({
  ...
  const usableGroups = Object.keys(usableGroup).filter((key) => key !== '');
  const groups = [
    'all',
    ...usableGroups.filter((group) => !CLAUDE_DISPLAY_GROUPS.has(group)),
    ...CLAUDE_DISPLAY_GROUP_ORDER.filter((group) =>
  ...
  const items = groups.map((g) => {
    ...
    return {
      value: g,
      label: g === 'all' ? t('全部分组') : usableGroup[g] || g,
