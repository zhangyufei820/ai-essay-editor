...
const PRICING_DISPLAY_OVERRIDES = {
  ...
  'ecommerce-banana-2': {
    ...
    description: '电商特价banana-2：仅支持 1K 输出，可编辑图像，人民币 ¥0.085/张。',
  ...
  'gpt-image-2-4K': {
    ...
    description: '星人 OpenAI 图像生成，支持 1K/2K/4K 与官方约束内自定义尺寸；gpt-image-2 最大边 3840，4K 横竖图为 3840x2160 / 2160x3840。',
  ...
  'geek2api-image-2': {
    ...
    description: '星人 Image 2：按 gpt-image-2 官方尺寸约束提交，支持 1K/2K/4K 与自定义 WxH，人民币 1K ¥0.03、2K ¥0.06、4K ¥0.10/张。',
...
export function getPricingDisplayOverride(modelOrName) {
  const modelName =
    typeof modelOrName === 'string'
      ? modelOrName
      : modelOrName?.model_name;
  return PRICING_DISPLAY_OVERRIDES[modelName] || null;
}

export function applyPricingDisplayOverrides(model) {
...
export function getPricingModelDisplayName(model) {
  return getPricingDisplayOverride(model)?.display_name || model?.display_name || model?.model_name || '';
}

export function getPricingModelDescription(model) {
...
export const calculateModelPrice = ({
  ...
  const displayOverride = getPricingDisplayOverride(record);
  // 1. 选择实际使用的分组
  ...
  if (displayOverride?.fixed_price_label) {
    return {
