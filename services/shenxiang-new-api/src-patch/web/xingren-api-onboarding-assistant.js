(function () {
    ...
    var actions = [
      ...
      },
      {
        ...
        text:
          "已经打开" +
          logTitle +
          "。\n\n这里用来看请求记录、任务状态、模型、时间和消耗。你要查今天的图片或媒体记录时，先看时间列，再按模型名或任务状态筛选；如果页面提供搜索框，我可以继续帮你点筛选项。",
        options: {
          tone: "operation",
          actions: [
  ...
  function findDockTargetNav() {
    var navs = document.querySelectorAll("header nav");
    for (var i = 0; i < navs.length; i += 1) {
      var nav = navs[i];
      if (navIsVisible(nav)) return nav;
    }
    return null;
  }

  function undockRoot(root) {
    if (root.parentElement !== document.body) {
      document.body.appendChild(root);
    }
    root.classList.remove("xr-api-assistant-docked");
    root.removeAttribute("data-xr-docked");
  }

  function dockRootInHeader(root) {
    var nav = findDockTargetNav();
    if (!nav) {
      if (root.classList.contains("xr-api-assistant-docked")) {
        undockRoot(root);
      }
      return false;
    }
    if (root.parentElement !== nav || root !== nav.firstChild) {
      nav.insertBefore(root, nav.firstChild);
    }
    root.classList.add("xr-api-assistant-docked");
    root.setAttribute("data-xr-docked", "top-nav");
    return true;
  }

  function refreshDockRoot(root) {
    if (root.parentElement && root.parentElement.tagName === "NAV" && !navIsVisible(root.parentElement)) {
      undockRoot(root);
    }
    return dockRootInHeader(root);
  }

  function scheduleDockRoot(root) {
    refreshDockRoot(root);
    if (dockTimer) return;
    var attempts = 0;
    dockTimer = window.setInterval(function () {
      attempts += 1;
      refreshDockRoot(root);
      if (attempts >= 40) {
  ...
  function startDockKeepAlive() {
    if (dockKeepAliveTimer) return;
    dockKeepAliveTimer = window.setInterval(function () {
      var root = document.getElementById("xr-api-assistant-root");
      if (root) refreshDockRoot(root);
    }, 1500);
  }
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      ".xr-api-assistant-launcher::after{content:'';position:absolute;right:8px;top:7px;width:8px;height:8px;border-radius:999px;background:#10b981;box-shadow:0 0 0 3px rgba(16,185,129,.16);pointer-events:none}",
      ".xr-api-assistant-docked .xr-api-assistant-launcher::after{right:6px;top:5px;width:7px;height:7px}",
      ".xr-api-assistant-launcher:hover{transform:translateY(-1px);border-color:#14b8a6;background:#ffffff;box-shadow:0 14px 34px rgba(15,23,42,.2)}",
      ".xr-api-assistant-docked .xr-api-assistant-launcher:hover{background:var(--semi-color-fill-0,rgba(15,23,42,.06));box-shadow:none;border-color:transparent}",
      ".xr-api-assistant-launcher-icon,.xr-api-assistant-avatar{display:grid;place-items:center;width:32px;height:32px;border-radius:999px;background:#111827;color:white;font-weight:850;font-size:12px;letter-spacing:0;overflow:hidden;box-shadow:0 5px 14px rgba(15,23,42,.22);flex:0 0 auto}",
      ".xr-api-assistant-docked .xr-api-assistant-launcher-icon{width:26px;height:26px;font-size:10px;box-shadow:none}",
      ...
      ".xr-api-assistant-docked .xr-api-assistant-launcher-copy{padding-right:0}.xr-api-assistant-docked .xr-api-assistant-launcher-copy strong{font-size:13px;line-height:1.2}.xr-api-assistant-docked .xr-api-assistant-launcher-copy small{display:none}",
