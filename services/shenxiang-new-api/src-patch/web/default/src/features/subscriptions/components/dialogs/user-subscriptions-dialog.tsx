...
function formatPlanOptionLabel(
  ...
  if (isValueMonthlyCard(plan)) {
    return `${plan.title || t('Monthly Card')} · ¥500 · ${t('30 days')} · ${t('Daily')} $${officialUsdFromQuota(Number(plan.total_amount || 0))} · ${t('Monthly')} $${officialUsdFromQuota(Number(plan.monthly_amount_total || 0))}`
  }
  return `${plan.title} ($${Number(plan.price_amount || 0).toFixed(2)})`
}
              ...
              <Select
                items={[
                  ...plans.map((p) => ({
                    ...
                    label: formatPlanOptionLabel(p.plan, t),
                  })),
                ]}
                  ...
                  <SelectGroup>
                    {plans.map((p) => (
                      <SelectItem key={p.plan.id} value={String(p.plan.id)}>
                        {formatPlanOptionLabel(p.plan, t)}
                      </SelectItem>
                    ))}
                  </SelectGroup>
                </SelectContent>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    subs.map((record) => {
                      ...
                      const monthlyTotal = Number(sub.monthly_amount_total || 0)
                      const monthlyUsed = Number(sub.monthly_amount_used || 0)
                      const monthlyCard = monthlyTotal > 0
                        ...
                        <TableRow key={sub.id}>
                          ...
                          <TableCell>
                            {monthlyCard ? (
                              <div className='text-sm'>
                                <div>
                                  {t('Daily')}: ${officialUsdFromQuota(used)} / $
                                  {officialUsdFromQuota(total)}
                                </div>
                                <div className='text-muted-foreground'>
                                  {t('Monthly')}: $
                                  {officialUsdFromQuota(monthlyUsed)} / $
                                  {officialUsdFromQuota(monthlyTotal)}
                                </div>
                              </div>
                            ) : total > 0 ? (
                              `${used}/${total}`
