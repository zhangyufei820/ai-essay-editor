...
export type ImageBenefitResponse = ApiResponse<ImageBenefit>
...
/**
 * Redemption code result. Older servers returned a number directly; newer
 * servers return this object so subscription activation can be displayed
 * clearly after code redemption.
 */
export interface RedemptionResult {
  /** Redemption benefit type */
  type?: 'quota' | 'subscription' | 'image_benefit'
  /** Quota added for balance codes */
  quota?: number
  /** Subscription plan ID for plan codes */
  plan_id?: number
  /** Subscription plan title for plan codes */
  plan_title?: string
  /** Created user subscription ID */
  user_subscription_id?: number
  /** Dedicated 1.5K image benefit package */
  image_benefit?: ImageBenefit
}

export interface ImageBenefit {
  token_id?: number
  key?: string
  name?: string
  base_url?: string
  ...
  expired_time?: number
  created_time?: number
  status?: 'active' | 'expired' | 'exhausted' | 'disabled' | 'none' | string
