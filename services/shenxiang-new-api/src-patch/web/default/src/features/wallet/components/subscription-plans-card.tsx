...
const QUOTA_PER_OFFICIAL_USD = 500000

function officialUsdFromQuota(quota: number): string {
  const usd = quota / QUOTA_PER_OFFICIAL_USD
  return Number.isInteger(usd) ? usd.toFixed(0) : usd.toFixed(2)
}
...
function isValueMonthlyCard(plan: PlanRecord['plan']): boolean {
  return (
    (plan.title || '').includes('500 月卡') ||
    Number(plan.monthly_amount_total || 0) > 0
  )
}
...
export function SubscriptionPlansCard({
  ...
  useEffect(() => {
    ...
    init()
  }, [fetchPlans, fetchSelfSubscription, refreshSignal])
              ...
              <div className='max-h-64 space-y-3 overflow-y-auto pr-1'>
                {allSubscriptions.map((sub) => {
                  ...
                  const monthlyTotalAmount = Number(
                    subscription?.monthly_amount_total || 0
                  )
                  const monthlyUsedAmount = Number(
                    subscription?.monthly_amount_used || 0
                  )
                  const remainAmount =
                    totalAmount > 0 ? Math.max(0, totalAmount - usedAmount) : 0
                  const monthlyRemainAmount =
                    monthlyTotalAmount > 0
                      ? Math.max(0, monthlyTotalAmount - monthlyUsedAmount)
                      : 0
                  const planTitle =
                    planTitleMap.get(subscription?.plan_id) || ''
                  const remainDays = getRemainingDays(sub)
                  const usagePercent = getUsagePercent(sub)
                  const monthlyUsagePercent = clampPercent(
                    monthlyUsedAmount,
                    monthlyTotalAmount
                  )
                  const monthlyCard = monthlyTotalAmount > 0
                  const now = Date.now() / 1000
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className='text-muted-foreground mt-1'>
                        {monthlyCard ? (
                          <div className='space-y-2'>
                            <div>
                              {t('Daily Quota')}: $
                              {officialUsdFromQuota(usedAmount)} / $
                              {officialUsdFromQuota(totalAmount)} ·{' '}
                              {t('Remaining')} $
                              {officialUsdFromQuota(remainAmount)} · {t('Used')}{' '}
                              {usagePercent}%
                            </div>
                            ...
                            <div>
                              {t('Monthly Quota')}: $
                              {officialUsdFromQuota(monthlyUsedAmount)} / $
                              {officialUsdFromQuota(monthlyTotalAmount)} ·{' '}
                              {t('Remaining')} $
                              {officialUsdFromQuota(monthlyRemainAmount)} ·{' '}
                              {t('Used')} {monthlyUsagePercent}%
                            </div>
                            {monthlyTotalAmount > 0 && isActive && (
                              <Progress
                      ...
                      {!monthlyCard && totalAmount > 0 && isActive && (
        ...
        {plans.length > 0 ? (
          <div className='grid grid-cols-1 gap-3 2xl:grid-cols-2 2xl:gap-4'>
            {plans.map((p, index) => {
              ...
              const monthlyAmount = Number(plan.monthly_amount_total || 0)
              const concurrencyLimit = Number(plan.concurrency_limit || 0)
              const monthlyCard = isValueMonthlyCard(plan)
              const price = monthlyCard
                ? '500'
                : Number(plan.price_amount || 0).toFixed(2)
              const pricePrefix = monthlyCard ? '¥' : '$'
              const isPopular = index === 0 && plans.length > 1
              const limit = Number(plan.max_purchase_per_user || 0)
              const count = planPurchaseCountMap.get(plan.id) || 0
              const reached = limit > 0 && count >= limit

              const benefits = monthlyCard
                ? [
                    `${t('Daily Quota')}: $${officialUsdFromQuota(totalAmount)} ${t('Official equivalent')}`,
                    `${t('Monthly Quota')}: $${officialUsdFromQuota(monthlyAmount)}`,
                    `${t('Validity Period')}: ${formatDuration(plan, t)}`,
                    `${t('Concurrency')}: ${concurrencyLimit || 5}`,
                    t('适合 Codex 重度开发使用'),
                  ]
                : ([
                    `${t('Validity Period')}: ${formatDuration(plan, t)}`,
                  ...
                  <CardContent className='flex h-full flex-col p-3.5 sm:p-4'>
                    ...
                    {reached ? (
                      ...
                      <Button
                        ...
                        className='w-full'
                        onClick={() => {
                          if (monthlyCard && !enableOnlineTopUp) {
                            const input =
                              document.getElementById('redemption-code')
                            ...
                            toast.info(t('Use a redemption code to activate this plan'))
                            return
                          }
                          setSelectedPlan(p)
                      ...
                      >
                        {monthlyCard && !enableOnlineTopUp
                          ? t('Activate with code')
                          : t('Subscribe Now')}
                      </Button>
                    )}
                  </CardContent>
