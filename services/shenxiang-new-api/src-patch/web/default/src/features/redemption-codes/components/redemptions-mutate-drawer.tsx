...
export function RedemptionsMutateDrawer({
  ...
  useEffect(() => {
    ...
    getAdminPlans().then((result) => {
      if (result.success && result.data) {
        setPlans(result.data.filter((record) => record.plan.enabled))
      }
    })
  }, [open])
  ...
  const selectedPlan = plans.find(
    (record) => record.plan.id === form.watch('plan_id')
  )?.plan
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        if (value === 'subscription') {
                          ...
                          if (form.getValues('quota_dollars') <= 0) {
                            form.setValue('quota_dollars', 10)
                          }
                        }
                      }}
                    >
                    ...
                    <FormDescription>
                      {t(
                        'Choose whether this code adds balance or activates a plan.'
                      )}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>
                        {selectedPlan
                          ? t('This code activates {{plan}} for the redeemer.', {
                              plan: selectedPlan.title,
                            })
                          : t('Only enabled plans can be selected.')}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
