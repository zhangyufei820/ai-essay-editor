...
export async function createImageTask(payload: Record<string, unknown>) {
  const res = await api.post<ImageTaskCreateResponse>(
...
export async function createImageEditTask(payload: FormData) {
  const res = await api.post<ImageTaskCreateResponse>(
...
export async function fetchImageTask(taskId: string) {
  const res = await api.get<ImageTaskFetchResponse>(
    `/pg/images/tasks/${encodeURIComponent(taskId)}`,
