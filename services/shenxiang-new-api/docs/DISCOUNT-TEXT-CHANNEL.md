...
三个独立渠道固定使用 `discount` 分组和 `0.25` 倍率，只暴露 `gpt-5.5`、`gpt-5.6-sol`、`gpt-5.6-terra`。主渠道必须完整支持三个模型；fallback 只启用各自 `/v1/models` 实际声明的目标模型子集。它们不加入自动分组。
...
默认优先级为 `zwaca → tkfora → aihub`，对应固定 channel `28 → 42 → 41` 和优先级 `30 → 20 → 10`。zwaca 与 aihub 使用原生 OpenAI Responses；tkfora 上游使用 Chat Completions，New API 仅在 `discount` 的 channel `42` 将公开 Responses 请求做兼容转换，并保留 `function_call` / `function_call_output` 的工具续接。`gpt-5.4-mini`、`gpt-5.6` 和 `gpt-5.6-luna` 不属于正式 0.25x 模型集合，不得写入渠道模型或启用 ability。运行脚本时可用 `--order zwaca,aihub,tkfora` 等完整排列切换顺序，禁止把三把不同成本或不同供应商密钥合并到同一个随机轮询渠道。
...
```text
DISCOUNT_ZWACA_API_KEY=主渠道密钥（可兼容旧 DISCOUNT_UPSTREAM_API_KEY）
DISCOUNT_TKFORA_API_KEY=tkfora fallback 密钥（可兼容旧 DISCOUNT_WANGWANG_API_KEY）
...
3. 全量维护时不带 `--apply` 运行 `scripts/configure_discount_text_channel.py`，确认主渠道完整包含三个目标模型，fallback 只包含其声明的目标模型子集。
4. 只替换 tkfora fallback 时先运行 `scripts/configure_discount_text_channel.py --channel tkfora`，再运行 `scripts/configure_discount_text_channel.py --channel tkfora --apply`；该模式只允许原子更新 channel `42` 及其 `discount` abilities，不改 options、channel `28` 或 channel `41`。
