...
CHANNEL_TAG = "xingren-gpt6-astra"
CHANNEL_NAME = "GPT-6 Astra 独立通道"
CHANNEL_GROUP = "astra"
CHANNEL_REMARK = "GPT-6 Astra 固定人民币计费通道"
EXPECTED_UPSTREAM_BASE_URL = "https://aihub.top"
UPSTREAM_KEY_ENV = "GPT6_ASTRA_UPSTREAM_API_KEY"
...
def normalize_base_url(value: str) -> str:
    ...
    if (
        ...
        raise ConfigurationError("the configured GPT-6 Astra endpoint is not permitted")
...
def validate_upstream_key(value: str, *, env_name: str | None = None) -> str:
    ...
    if len(key) < 16 or any(character.isspace() for character in key):
        raise ConfigurationError(f"{env_name or 'configured GPT-6 Astra credential'} is missing or invalid")
    return key
...
def fetch_json(url: str, api_key: str, *, body: dict[str, object] | None = None) -> dict[str, object]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "shenxiang-gpt6-astra-probe/1.0",
        },
    ...
    for attempt in range(1, MAX_REQUEST_ATTEMPTS + 1):
        try:
            ...
            if exc.code not in RETRYABLE_HTTP_STATUS or attempt == MAX_REQUEST_ATTEMPTS:
                raise ConfigurationError(f"GPT-6 Astra upstream request returned HTTP {exc.code}") from None
        except (urllib.error.URLError, TimeoutError):
            if attempt == MAX_REQUEST_ATTEMPTS:
                raise ConfigurationError("GPT-6 Astra upstream request failed or timed out") from None
        time.sleep(2 ** (attempt - 1))
    if len(raw) > MAX_RESPONSE_BYTES:
        raise ConfigurationError("GPT-6 Astra upstream response exceeded the size limit")
    try:
        payload = json.loads(raw)
    except (json.JSONDecodeError, UnicodeDecodeError):
        raise ConfigurationError("GPT-6 Astra upstream response was not valid JSON") from None
    if not isinstance(payload, dict):
        raise ConfigurationError("GPT-6 Astra upstream response was not an object")
    return payload


def verify_upstream(base_url: str, api_key: str) -> None:
    normalized = normalize_base_url(base_url)
    models = fetch_json(normalized + "/v1/models", api_key)
    available = {
        item.get("id")
    ...
    if MODEL_NAME not in available:
        raise ConfigurationError("the required GPT-6 Astra model is not available")
    ...
    if response.get("status") != "completed" or response_text != "OK":
        raise ConfigurationError("GPT-6 Astra Responses verification did not complete")
    ...
    if not isinstance(choices, list) or not choices or not isinstance(choices[0], dict):
        raise ConfigurationError("GPT-6 Astra chat verification did not contain a choice")
    message = choices[0].get("message")
    if not isinstance(message, dict) or str(message.get("content") or "").strip() != "OK":
        raise ConfigurationError("GPT-6 Astra chat verification did not complete")
...
def load_existing_channel() -> tuple[str, str] | None:
    ...
    if len(rows) != 1 or len(rows[0]) != 2:
        raise ConfigurationError("the configured GPT-6 Astra channel is ambiguous")
...
def validate_channel_isolation() -> None:
    ...
    if (int(rows[0][0]) if rows else 0) > 1:
        raise ConfigurationError("multiple channels use the reserved GPT-6 Astra tag")
...
def build_apply_sql(api_key: str, base_url: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "SET @astra_channel_id := (SELECT MIN(id) FROM channels WHERE tag = " + sql_quote(CHANNEL_TAG) + ");",
            ...
            + " WHERE @astra_channel_id IS NULL;",
            "SET @astra_channel_id := IFNULL(@astra_channel_id, LAST_INSERT_ID());",
            ...
            + ", remark = "
            + sql_quote(CHANNEL_REMARK)
            + " WHERE id = @astra_channel_id;",
            "UPDATE channels SET status = 2 WHERE id <> @astra_channel_id AND FIND_IN_SET("
            + sql_quote(MODEL_NAME)
            + ", REPLACE(COALESCE(models, ''), ' ', '')) > 0;",
            "UPDATE abilities SET enabled = 0 WHERE model = "
            + sql_quote(MODEL_NAME)
            + " AND channel_id <> @astra_channel_id;",
            "UPDATE abilities SET enabled = 0 WHERE channel_id = @astra_channel_id AND model <> "
            + sql_quote(MODEL_NAME)
            + ";",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Safely configure the isolated GPT-6 Astra channel")
    action = parser.add_mutually_exclusive_group()
    action.add_argument("--apply", action="store_true", help="validate and write the GPT-6 Astra channel")
    action.add_argument("--reconcile-if-configured", action="store_true", help="verify and reapply only an existing GPT-6 Astra channel")
