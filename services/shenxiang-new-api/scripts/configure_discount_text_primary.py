...
PRIMARY_TAG = "xingren-discount-text-geek2api"
FALLBACKS = (
...
def build_apply_sql(api_key: str, base_url: str) -> str:
    tags = (PRIMARY_TAG, *(tag for tag, _priority in FALLBACKS))
    ...
    statements = [
        "START TRANSACTION;",
        "SELECT `value` FROM options WHERE `key` = 'GroupRatio' FOR UPDATE;",
        ...
        "SET @primary_count := (SELECT COUNT(*) FROM channels WHERE tag = " + sql_quote(PRIMARY_TAG) + ");",
        ...
        "SET @route_apply_status := CASE WHEN @ratio_ok <> 1 THEN 'group_ratio_invalid' WHEN @primary_count > 1 THEN 'primary_duplicate' WHEN @fallback_count <> " + str(len(FALLBACKS)) + " THEN 'fallback_invalid' ELSE 'ok' END;",
        "SET @route_apply_allowed := IF(@route_apply_status = 'ok', 1, 0);",
        "SET @primary_id := (SELECT MIN(id) FROM channels WHERE tag = " + sql_quote(PRIMARY_TAG) + ");",
        "INSERT INTO channels (type, `key`, status, name, weight, created_time, test_time, response_time, base_url, models, `group`, model_mapping, priority, auto_ban, tag, remark) SELECT 1," + sql_quote(api_key) + ",1," + sql_quote(PRIMARY_NAME) + ",100,UNIX_TIMESTAMP(),0,0," + sql_quote(base_url) + "," + sql_quote(models) + ",'discount'," + sql_quote(mapping) + ",30,1," + sql_quote(PRIMARY_TAG) + ",'0.25x text primary' WHERE @primary_id IS NULL AND @route_apply_allowed = 1;",
        "SET @primary_id := IF(@route_apply_allowed = 1, IFNULL(@primary_id, LAST_INSERT_ID()), NULL);",
        "UPDATE channels SET type=1, `key`=" + sql_quote(api_key) + ", status=1, name=" + sql_quote(PRIMARY_NAME) + ", weight=100, base_url=" + sql_quote(base_url) + ", models=" + sql_quote(models) + ", `group`='discount', model_mapping=" + sql_quote(mapping) + ", priority=30, auto_ban=1, tag=" + sql_quote(PRIMARY_TAG) + " WHERE id=@primary_id AND @route_apply_allowed=1;",
        "UPDATE channels SET priority=20 WHERE tag=" + sql_quote(FALLBACKS[0][0]) + " AND @route_apply_allowed=1;",
        "UPDATE channels SET priority=10 WHERE tag=" + sql_quote(FALLBACKS[1][0]) + " AND @route_apply_allowed=1;",
