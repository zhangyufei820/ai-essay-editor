...
UPSTREAM = permissions.UPSTREAM_MOON25_B720_VIDEO_MODEL
TAG = permissions.PUBLIC_MOON25_B720_VIDEO_CHANNEL_TAG
GROUPS = "default,standard,pro,code,internal"
PRICE_CNY = permissions.Decimal("0.55")
...
def stage(key: str) -> dict[str, str]:
    quote = permissions.sql_quote
    rows = channel_rows()
    if rows:
        raise ValueError("channel already exists; inspect it before staging again")
    conflicts = permissions.mysql(
        "SELECT COUNT(*) FROM channels WHERE FIND_IN_SET("
        + quote(MODEL)
    ...
    tokens = permissions.mysql_raw(
        "SELECT id, `key`, model_limits FROM tokens WHERE user_id=1 "
        "AND name='星人视频生成令牌' AND deleted_at IS NULL AND status=1"
    )
    if len(tokens) != 1:
    ...
    snapshot = {
        ...
        "model": MODEL,
        "channels": rows,
        "previous_price": prices.get(MODEL),
        "admin_token_id": token_id,
        "admin_model_limits": limits,
    }
    log_path = permissions.ROOT / "logs" / (
        "moon25-b720-before-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S") + ".json"
    )
    with log_path.open("x", encoding="utf-8") as stream:
        json.dump(snapshot, stream, ensure_ascii=False, indent=2)

    prices[MODEL] = permissions.decimal_to_float(PRICE_CNY / permissions.usd_exchange_rate())
    ...
    values = [
        ...
        1,
        TAG,
        "人民币 ¥0.55/秒；720P；4-30秒；30图、10视频、10音频；default、按秒、openai-video",
    ]
    sql = [
        "START TRANSACTION;",
        "INSERT INTO channels (type, `key`, status, name, weight, base_url, models, `group`, model_mapping, priority, auto_ban, tag, remark, created_time, test_time, response_time) VALUES ("
        + ",".join(str(value) if isinstance(value, int) else quote(value) for value in values)
        + ", UNIX_TIMESTAMP(),0,0);",
        "SET @moon_b720_channel := LAST_INSERT_ID();",
        "INSERT INTO abilities (`group`, model, channel_id, enabled, priority, weight, tag) VALUES ("
        + ",".join([quote("internal"), quote(MODEL), "@moon_b720_channel", "1", "0", "100", quote(TAG)])
        + ");",
...
def publish() -> dict[str, object]:
    ...
    successes = permissions.mysql(
        "SELECT COUNT(*) FROM tasks WHERE user_id=1 AND channel_id="
        + str(int(rows[0][0]))
        + " AND status='SUCCESS' AND properties LIKE '%"
        + MODEL
        + "%'"
    )
    ...
    permissions.mysql_exec(
        "UPDATE channels SET `group`=" + permissions.sql_quote(GROUPS) + " WHERE id=" + str(int(rows[0][0])) + ";"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage or publish Moon Video 2.5 720P")
    ...
    with open("/tmp/shenxiang-new-api-model-sync.lock", "a+") as lock:
        ...
        if args.action == "stage":
            key = validate_api_key(os.environ.get("MOON25_B720_API_KEY") or getpass.getpass("Upstream key: "))
            result = stage(key)
        else:
            result = publish()
