...
def build_sql(env: dict[str, str], keys: dict[str, str], updates: dict[str, str]) -> str:
    ...
    statements.extend(
        [
            ...
            "UPDATE tokens SET `group` = " + sql_quote("claude-external") + ", model_limits_enabled = 1, model_limits = " + sql_quote(",".join(CHANNELS[3]["models"])) + " WHERE deleted_at IS NULL AND name = " + sql_quote("星人 Claude 高阶令牌") + ";",
            "UPDATE tokens SET `group` = " + sql_quote("claude-external") + ", model_limits_enabled = 1, model_limits = " + sql_quote(",".join(CHANNELS[3]["models"])) + " WHERE deleted_at IS NULL AND LOWER(TRIM(name)) = 'claude';",
            "COMMIT;",
        ]
    )
    return "\n".join(statements) + "\n"
