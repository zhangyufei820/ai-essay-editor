...
describe('Sprint 5 payment / credits / membership guards', () => {
  ...
  it('xunhupay callback rejects bad signatures and validates amount / credits before granting权益', () => {
    ...
    expect(source).toContain('grantPaymentCreditsWithOptimisticRetry')
    expect(source).toContain('restoreClaimedOrderToPending')
    expect(source).toContain('.eq("status", "paid")')
    const creditGrant = read('lib/payment-credit-grant.ts')
    expect(creditGrant).toContain('balanceAfter > MAX_PAYMENT_CREDITS_BALANCE')
    expect(creditGrant).toContain('.eq("credits", currentCredits.credits)')
    expect(creditGrant).toContain('.select("credits")')
    expect(creditGrant).toContain('credits insert conflict, retrying as increment')
