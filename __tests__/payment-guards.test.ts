...
describe('Sprint 5 payment / credits / membership guards', () => {
  ...
  it('payment creation persists credits_amount from product catalog', () => {
    ...
    expect(source).toContain('rejectUntrustedOrigin(request)')
    expect(source).toContain('export async function POST(request: NextRequest)')
    expect(source).not.toContain('export async function GET(request: NextRequest)')
    expect(source).toContain('crypto.randomBytes(16).toString("hex")')
    expect(source).toContain('signal: AbortSignal.timeout(15_000)')
    expect(source).not.toContain('请求迅虎支付参数:", params')
    ...
    expect(checkout).toContain('method: "POST"')
    expect(checkout).toContain('body: JSON.stringify({ productId, billing })')
  })

  it('applies the same server-side product and membership gates to Stripe checkout', () => {
    const source = read('app/api/stripe/checkout-session/route.ts')

    expect(source).toContain('requireLearningUserId(request)')
    expect(source).toContain('rejectUntrustedOrigin(request)')
    expect(source).toContain('getUserEntitlementSummary(userId')
    expect(source).toContain('validateProductPurchase(productId, entitlement?.membershipStatus || null)')
    expect(source).not.toContain('body.userId')

    const webhook = read('app/api/stripe/webhook/route.ts')
    expect(webhook).toContain('session.payment_status !== "paid"')
    expect(webhook).toContain('session.amount_total !== expectedAmountInCents')
    expect(webhook).toContain('session.currency?.toLowerCase() !== "cny"')
  })
  ...
  it('xunhupay callback rejects bad signatures and validates amount / credits before granting权益', () => {
    ...
    expect(source).toContain('grantPaymentCreditsOnce(supabase')
    expect(source).not.toContain('restoreClaimedOrderToPending')
    expect(source).not.toContain('grantPaymentCreditsWithOptimisticRetry')
    const migration = read('scripts/024_payment_credit_idempotency.sql')
    expect(migration).toContain('CREATE TABLE IF NOT EXISTS public.payment_credit_events')
    expect(migration).toContain('CREATE OR REPLACE FUNCTION public.grant_payment_credits_once')
    expect(migration).toContain('p_user_id TEXT')
    expect(migration).toContain('ALTER COLUMN reference_id TYPE TEXT')
    expect(migration).toContain('ADD COLUMN IF NOT EXISTS balance_before INTEGER')
    expect(migration).toContain('ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ')
    expect(migration).toContain('VALUES (p_user_id, 0, FALSE)')
    expect(migration).toContain('FOR UPDATE')
    expect(migration).toContain("ON CONFLICT DO NOTHING")
    expect(migration).toContain("SET status = 'paid'")
    expect(migration).toContain("AND status = 'pending'")
    expect(migration).toContain('payment credit balance limit exceeded')
    expect(source).not.toContain('签名验证失败，但继续处理订单')
    expect(source).toContain('XUNHUPAY_NOTIFY_MAX_BYTES')
    expect(read('lib/xunhupay.ts')).toContain('crypto.timingSafeEqual')
  })
  ...
  it('routes GPT Image 2 through the direct image gateway after server-side billing guards', () => {
    ...
    const directGatewayIndex = source.indexOf('console.log("🎨 [GPT Image] 使用统一图片任务网关，绕过 Dify chatflow")')
    ...
    expect(source).toContain('gatewayName: process.env.SHENXIANG_NEW_API_IMAGE_API_KEY ? "managed-image-gateway" : "dify-image-gateway"')
    expect(source).toContain('NEW_API_IMAGE_BASE_URL')
  ...
  it('routes Gemini image through the dedicated gateway after server-side billing guards', () => {
    ...
    expect(source).toContain('"https://api.aiphui.top"')
    expect(source).toContain('function getGeminiImageGatewayCandidates')
    expect(source).toContain('GEMINI_IMAGE_GATEWAY_MANAGED_COMPAT')
    ...
    expect(source).toContain('/v1/images/generations')
    expect(source).toContain('/api/image/unified')
  ...
  it('keeps standalone essay image upload authenticated', () => {
    ...
    const submitStart = source.indexOf('/api/essay-grade')
    const submitBlock = source.slice(Math.max(0, submitStart - 250), submitStart + 350)
    expect(submitBlock).toContain('...(await getRequiredAuthHeaders())')
    expect(source).toContain('登录状态已过期，请重新登录后再提交。')
