...
describe("chat think rendering and composer layout", () => {
  ...
  it("keeps local history sessions separate from Dify memory conversations", () => {
    ...
    expect(chatSessionRoute).toContain("const effectiveSessionId = requestedId || randomUUID()")
    expect(chatSessionRoute).toContain("id: effectiveSessionId")
  ...
  it("keeps OpenClaw final node output visible when message chunks are empty", () => {
    ...
    expect(route).toContain("emitOpenClawFallbackDisplay(controller)")
    expect(route).toContain("enqueueSseAnswer(controller, preferredText)")
