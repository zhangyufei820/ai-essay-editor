...
describe("codex skill gateway intent routing", () => {
  ...
  it("serves task files inline by default and indexes generated artifacts as preview links", () => {
    ...
    expect(main).toContain('"file_token": secrets.token_urlsafe(32)')
    expect(main).toContain('secrets.compare_digest(provided_token, expected_token)')
    ...
    expect(appProxy).toContain('searchParams.get("token")')
    expect(appProxy).toContain('upstreamUrl.searchParams.set("token", fileToken)')
