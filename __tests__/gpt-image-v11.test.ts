...
describe("GPT Image V11 parameter mapping", () => {
  ...
  it("keeps Image 2 direct gateway sizes normalized before submission", () => {
    ...
    expect(routeSource).toContain("function normalizeVivaApiImageModel")
    expect(routeSource).toContain('if (!trimmed || trimmed === "gpt-image-2-vip") return "gpt-image-2"')
    expect(routeSource).toContain("const VIVAAPI_IMAGE_MODEL = normalizeVivaApiImageModel(process.env.VIVAAPI_IMAGE_MODEL)")
    expect(routeSource).toContain("function getVivaApiImageGatewayCandidates")
    expect(routeSource).toContain("MOONAPIX_LLM_API_KEY")
    expect(routeSource).toContain("VIVAAPI_LLM_API_KEY")
    ...
    expect(routeSource).toContain("图像服务返回空白图片，正在自动重试")
    ...
    expect(routeSource).toContain("图像任务已提交")
    ...
    expect(routeSource).toContain("shouldRetryBananaEdit1KWithAcceptedGatewaySize")
    expect(routeSource).toContain("shouldRetryBananaGatewayTimeout")
    expect(routeSource).toContain("banana_edit_1k_gateway_400")
    expect(routeSource).toContain("banana_gateway_timeout")
    expect(routeSource).toContain('requested_image_size: imageInputs.image_size')
    expect(routeSource).toContain('effective_image_size: retryInputs.image_size')
    expect(routeSource).toContain('effective_inputs: effectiveImageInputs')
    expect(routeSource).toContain("/v1/images/generations")
    ...
    expect(routeSource).toContain('if (imageInputs.quality) formData.append("quality", imageInputs.quality)')
    expect(routeSource).not.toContain("isGptImage2VipModel")
  ...
  it("submits Gemini and Banana image requests as async tasks to avoid edge timeouts", () => {
    ...
    expect(routeSource).toContain("const usesPersistedTaskRun = isDirectImageGatewayRequest && async_image_task === true")
    expect(routeSource).toContain("const taskRun = usesPersistedTaskRun")
