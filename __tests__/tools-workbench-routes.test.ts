...
describe("tools workbench route mappings", () => {
  ...
  it("keeps all browser tool entries mapped to local API routes", () => {
    ...
    expect(page).toContain('fetch("/api/voice/tts"')
    expect(page).toContain("URL.createObjectURL(audio)")
    expect(page).toContain('voice_id: "coral"')
    expect(page).not.toContain('fetch("/api/omnivoice/tts"')
    expect(page).not.toContain("/api/omnivoice/jobs/")
  ...
  it("maps text-to-speech through the OmniVoice server gateway", () => {
    ...
    const mediaRoute = read("app/api/omnivoice/media/[filename]/route.ts")
    const ownership = read("lib/omnivoice-job-ownership.ts")
    const migration = read("scripts/027_omnivoice_job_ownership.sql")
    expect(mediaRoute).toContain("fetchOmniMedia")
    expect(mediaRoute).toContain("requireUser(request)")
    expect(mediaRoute).toContain("isOmniVoiceMediaOwner")
    expect(voicesRoute).toContain("listOmniVoices")
    expect(ttsRoute).toContain("createOmniTtsJob")
    expect(ttsRoute).toContain("createOmniVoiceJobOwner")
    expect(jobRoute).toContain("getOmniTtsJob")
    expect(jobRoute).toContain("isOmniVoiceJobOwner")
    expect(ownership).toContain('.from("omnivoice_job_owners")')
    expect(migration).toContain("CREATE TABLE IF NOT EXISTS public.omnivoice_job_owners")
    expect(migration).toContain("REVOKE ALL ON public.omnivoice_job_owners")
  })
