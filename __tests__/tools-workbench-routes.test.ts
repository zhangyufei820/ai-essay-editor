...
describe("tools workbench route mappings", () => {
  it("routes OCR and document extraction through essay-ai-suite instead of provider-specific mocks", () => {
    ...
    expect(ocr).not.toContain('gateway: "essay-ai-suite"')
    ...
    expect(doc).not.toContain('gateway: "essay-ai-suite"')
    expect(doc).not.toContain("provider: result.result.provider")
  ...
  it("keeps all browser tool entries mapped to local API routes", () => {
    ...
    expect(page).toContain('fetch("/api/image-prompt/reverse?stream=1"')
    expect(page).toContain("readReversePromptStream")
    expect(page).toContain("getRequiredToolAuthHeaders")
    expect(page).toContain("clearStoredAuthState()")
    expect(page).toContain('response.status === 401')
    expect(page).toContain("buildToolsLoginRedirect")
    ...
    expect(page).toContain('label: "高清写实"')
    expect(page).toContain('label: "创意插画"')
    expect(page).toContain("isHtmlErrorContent")
    expect(page).toContain("图片服务暂时不可用，请稍后重试。")
    expect(page).not.toContain('label: "gpt-image-2"')
    expect(page).not.toContain('label: "nano_banana"')
    expect(page).not.toContain("参数映射")
    expect(page).not.toContain("Tools Desk")
    expect(page).not.toContain("Sparkpage 综合报告")
  ...
  it("keeps image prompt reverse server-side and maps both target model parameters", () => {
    ...
    expect(route).toContain("files: [imageFile]")
    expect(route).toContain("sanitizeReverseError")
    ...
    expect(route).toContain("生成方式参数错误，请刷新页面后重试")
    expect(route).not.toContain("参数错误：target_model 仅支持 gpt-image-2 或 nano_banana")
  ...
  it("does not expose upstream HTML error pages in image tool surfaces", () => {
    ...
    expect(toolPage).toContain("upstream_balance_exhausted")
    expect(imageWorkspace).toContain("upstream_balance_exhausted")
    expect(toolPage).toContain("图像服务额度不足，本次积分已自动退回")
    expect(imageWorkspace).toContain("图像服务额度不足，本次积分已自动退回")
    expect(`${toolPage}\n${imageWorkspace}`).not.toContain("图像上游额度不足")
  ...
  it("renders the worksheet diagnosis page with the full upload and poster generation app", () => {
    ...
    expect(app).toContain('htmlFor="worksheet-camera-upload"')
    expect(app).not.toContain("shouldUseSystemCameraPicker")
    expect(app).not.toContain("cameraInputRef.current?.click()")
