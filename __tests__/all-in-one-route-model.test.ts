...
describe("all-in-one route model sync", () => {
  ...
  it("routes Gemini image through its gateway instead of Dify workflow", () => {
    ...
    expect(text).toContain("async function callGeminiImageGatewayDirect(query: string, inputs: unknown)")
    expect(text).toContain("await callGeminiImageGatewayDirect(params.query, params.inputs)")
    expect(text).toContain("runGeminiImageGatewayTask({")
  ...
  it("keeps chat apps streaming by default while allowing explicit blocking fallbacks", () => {
    ...
    expect(text).toContain('const BEIKE_PRO_MODEL = "beike-pro"')
    expect(text).toContain("model === BEIKE_PRO_MODEL")
    expect(text).toContain("const FORCE_BLOCKING_DIFY_CHAT_MODELS = new Set<string>()")
    expect(text).not.toContain('  "quanquan-math",')
    expect(text).toContain('FORCE_BLOCKING_DIFY_CHAT_MODELS.has(model || "")')
